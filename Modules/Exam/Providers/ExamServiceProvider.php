...
class ExamServiceProvider extends ServiceProvider
{
    ...
    {
        $this->publishes([
            __DIR__.'/../Config/config.php' => config_path('exam.php'),
        ], 'config');
        $this->mergeConfigFrom(
            __DIR__.'/../Config/config.php', 'exam'
        );
    }
    ...
    public function registerViews()
    {
        ...
        }, \Config::get('view.paths')), [$sourcePath]), 'exam');
    }
    ...
    {
        $langPath = resource_path('lang/modules/exam');

        if (is_dir($langPath)) {
            $this->loadTranslationsFrom($langPath, 'exam');
        } else {
